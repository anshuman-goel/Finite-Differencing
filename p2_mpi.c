...
int main (int argc, char *argv[]){
        ...
        int chunk_size = (NGRID + 2)/numproc; //size for each chunk
        ...
        //Allocate space for function and derivative array values
        y = (double*) malloc(chunk_size*sizeof(double));
        dy = (double*) malloc(chunk_size*sizeof(double));
        //Set imin and imax values for each chunk
        if(rank == 0) {
        ...
        //Start measuring time for the send-receive calls
        start_send_call_measure = MPI_Wtime();
        // Storing the boundary f(x) values
        double left_end = 99, right_end = 99; //dummy values
        if(rank == 0) {
        ...
        //End measurement for send-receive-calls
        ...
        //Start measurement for error calculation
        ...
        if(rank==0) {
                ...
                // Appending error array received from all the processors to global-error-array
                ...
                //End measurement of error calculation
                error_end = MPI_Wtime();
                ...
                if (MANUALREDUCE == 1) {
                        //Start measurement for local minima and maxima calculation
                        start_dips = MPI_Wtime();
                        int global_ptr = 0;
                        //Add own local minima and maxima values to the global array
                        ...
                        //Add non-root processors's local minima and maxima values to global array
                        for(int i = 1; i < numproc; i++) {
